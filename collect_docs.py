...
SOURCES = [
    ...
    # 회사 산출물을 변환해 둔 것. **git 레포가 아니라 dev-01 의 디렉토리다.**
    #
    # 그래서 인덱싱 CronJob 은 이 소스를 못 본다 — 파드는 dev-01 파일시스템을
    # 볼 수 없다. 위 순회 로직이 경로 없으면 건너뛰므로 CronJob 이 깨지지는
    # 않고 "건너뜀(경로 없음)" 만 찍힌다. 손으로 돌릴 때만 들어간다.
    #
    # 레포로 만들지 않은 것은 의도적이다. 고객사 산출물이라 GitHub 에 올리는
    # 것은 결정이 더 무거워지고, 문서가 자주 바뀌지도 않아 자동 갱신이 필요
    # 없다. 필요 없어지면 source 로 골라서 지우면 된다.
    #
    #   collection.delete(where={"source": "nhmall-arch"})
    #
    # .hwp 원본은 넣지 않는다. 확장자로 걸러진다 — 바이너리라 넣어 봐야
    # 임베딩이 의미를 못 만든다. 변환된 마크다운만 쓴다.
    Source(
        alias="nhmall-arch",
        root=HOME / "incoming",
        exts={".md"},
        # hwp5proc 가 남긴 중간 산출물과 변환용 가상환경.
        exclude={"html/", ".venv"},
        category="docs",
    ),
]
